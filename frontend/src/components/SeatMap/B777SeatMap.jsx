...
// B777-300ER: 310 asientos
// Primera clase: filas 1-2, 2-2 layout (AB | CD)
// Turista: filas 4-34, 3-3-3 (ABC | DEF | GHJ) sin I
...
export default function B777SeatMap({ seatMap, onSeatClick }) {
  const firstRows = [1, 2]
  const ecoRows = Array.from({ length: 34 }, (_, i) => i + 4)  // rows 4-37
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-4 select-none">
      ...
      <div className="flex items-center gap-0.5 mb-0.5">
        ...
        <Aisle />
        {['G','H','J'].map(c => <span key={c} className="w-6 text-center text-xs text-slate-500">{c}</span>)}
      </div>
